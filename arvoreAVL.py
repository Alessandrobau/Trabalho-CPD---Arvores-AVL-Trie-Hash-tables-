...
class AVLtree:
    ...
    # inserir
    def insertOnAVL(self, base, key, value):
        if base is None:
            return Node(key, value) 
        if key == base.key:
            base.valuesList.append(value)
        elif key < base.key:
            base.left = self.insertOnAVL(
                base.left, key, value
            )  # inserido na esquerda se for menor
        else:
            base.right = self.insertOnAVL(base.right, key, value)  # na direita se for maior
    ...
    def insertNode(self, key, value):
        self.base = self.insertOnAVL(self.base, key, value)
    ...
    def searchRangeTraversal(self, node, min, max):
        if node:
            if min <= node.key: 
                ...
                return self.searchRangeTraversal(node.right, min, max)
        
    def addToList(self, node, max, valueList):
        if node:
            self.addToList(node.left, max, valueList)
            if node.key <= max:   
                valueList.extend(node.valuesList)
                self.addToList(node.right, max, valueList)

    def searchRange(self, min, max):
        return self.searchRangeTraversal(self.base, min, max)
